...
// Op with lowering_config should NOT get the attribute.
...
// CHECK-LABEL: @skip_lowering_config
func.func @skip_lowering_config(%arg0: tensor<512xf32>) -> tensor<512xf32> {
  %laid_out = iree_vector_ext.to_layout %arg0 to layout(#layout) : tensor<512xf32>
  %empty = tensor.empty() : tensor<512xf32>
  // CHECK: linalg.generic
  ...
  %result = linalg.generic {
    indexing_maps = [affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>],
    iterator_types = ["parallel"],
    lowering_config = #iree_gpu.lowering_config<{thread = [4]}>
  } ins(%laid_out : tensor<512xf32>) outs(%empty : tensor<512xf32>) {
  ^bb0(%in: f32, %out: f32):
    %neg = arith.negf %in : f32
    linalg.yield %neg : f32
  } -> tensor<512xf32>
